...
W = tf.get_variable("weight", [1,1],tf.float32 )
B = tf.get_variable("bias", [1,1],tf.float32)
...
with tf.Session() as sess:
    ...
    for _epoch in range(5):
        ...
        while begin_ind < len(shuffled_indices):
            print(begin_ind)
            sess.run(tf.train.AdamOptimizer(learning_rate = 0.1).minimize(calculate_cost(Y, X, W, B)),
                     feed_dict = {X: x[begin_ind:end_ind,None], Y: y[begin_ind:end_ind,None]})
            cost = sess.run(calculate_cost(y, x, w, b), 
                     feed_dict = {X: x[begin_ind:end_ind,None], Y: y[begin_ind:end_ind,None]})
            print(cost)
            begin_ind = end_ind
